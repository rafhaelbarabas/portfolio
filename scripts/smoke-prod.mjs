...
check("MCP initialize echoes the protocol and names the server", async () => {
  const body = await rpc({
    ...
    params: {
      ...
      clientInfo: { name: "smoke-prod" },
      capabilities: {},
    },
  });
  const r = body.result ?? {};
  if (r.protocolVersion !== "2025-06-18")
    fail(`protocolVersion: ${r.protocolVersion}`);
  if (r.serverInfo?.name !== "rubenmarcus-portfolio")
    fail(`serverInfo: ${preview(r.serverInfo)}`);
...
check("MCP notifications/initialized returns an empty 202", async () => {
  ...
  if (res.status !== 202 || text !== "")
    fail(`got ${res.status} with ${text.length} bytes`);
});
...
check("MCP book_intro validates without relaying", async () => {
  const body = await rpc({
    ...
    id: 4,
    method: "tools/call",
    params: { name: "book_intro", arguments: {} },
  });
  const text = body.result?.content?.[0]?.text ?? "";
  if (!text.startsWith("error:"))
    fail(`expected validation error, got: ${text.slice(0, 60)}`);
});
...
check("MCP error codes: -32602 / -32601 / -32700", async () => {
  ...
  if (unknownTool.error?.code !== -32602)
    fail(`unknown tool: ${preview(unknownTool)}`);
  const unknownMethod = await rpc({
    jsonrpc: "2.0",
    id: 6,
    method: "resources/list",
  });
  if (unknownMethod.error?.code !== -32601)
    fail(`unknown method: ${preview(unknownMethod)}`);
  const res = await get("/api/mcp", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: "{broken",
  ...
  if (res.status !== 400 || body.error?.code !== -32700)
    fail(`malformed: ${res.status} ${preview(body)}`);
});
...
check("/.well-known discovery documents answer", async () => {
  ...
  if (!catalog.res.headers.get("content-type")?.includes("linkset"))
    fail("api-catalog content-type");
  const card = await getJson("/.well-known/mcp/server-card.json");
  if (card.body.transport?.endpoint !== `${ORIGIN}/api/mcp`)
    fail(`server-card: ${preview(card.body.transport)}`);
  ...
  if (!/^sha256:[a-f0-9]{64}$/.test(skills.body.skills?.[0]?.digest ?? ""))
    fail("agent-skills digest");
});
...
check("curl user-agent on / gets the terminal resume", async () => {
  ...
  const text = await res.text();
  if (!text.includes("terminal resume"))
    fail(
      `got ${res.headers.get("content-type")}, first bytes: ${text.slice(0, 60)}`,
    );
});
...
check("trailing slash 308s and preserves the query", async () => {
  ...
  if (res.status !== 308 || !location.endsWith("/about?x=1"))
    fail(`got ${res.status} → ${location || "(none)"}`);
});
...
check("sitemap only advertises canonical (no-slash) URLs", async () => {
  ...
  if (slashed.length > 0)
    fail(`${slashed.length} slashed URLs, e.g. ${slashed[0]}`);
});
...
check("RSS item links follow the no-slash policy", async () => {
  for (const feed of ["/rss.xml", "/rss-pt.xml"]) {
    ...
    if (slashed.length > 0)
      fail(`${feed}: ${slashed.length} slashed links, e.g. ${slashed[0]}`);
  }
});
...
check("AEO surfaces answer with their content types", async () => {
  ...
  for (const [path, type] of surfaces) {
    ...
    if (res.status !== 200 || !contentType.includes(type))
      fail(`${path}: ${res.status} ${contentType}`);
  }
});
...
check("GET /api/views reads a real post counter (no write)", async () => {
  ...
  if (res.status !== 200 || typeof body.views !== "number")
...
check("GET /api/stats aggregates the observability tables", async () => {
  ...
  if (
    res.status !== 200 ||
    typeof body.views_total !== "number" ||
    typeof body.mcp_events_total !== "number"
  ) {
    fail(`${res.status} ${preview(body)}`);
  }
});
