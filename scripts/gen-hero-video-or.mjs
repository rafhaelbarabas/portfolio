...
while (status.status !== "completed" && status.status !== "failed") {
  ...
  if (status.status === "failed") {
    console.error(
      "FAILED:",
      status.error ?? JSON.stringify(status).slice(0, 500),
    );
    process.exit(1);
  }
}
