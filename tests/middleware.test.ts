...
describe("OAuth discovery metadata", () => {
  ...
  it("serves protected-resource metadata at both well-known paths", async () => {
    for (const p of [
      ...
      const res = requireResponse(
        await onRequest(ctxFor(p) as never, next as never),
      );
      const body = await res.json();
...
describe("agent discovery", () => {
  it("publishes a typed RFC 9727 API catalog", async () => {
    const res = requireResponse(
      await onRequest(
        ctxFor("/.well-known/api-catalog") as never,
        next as never,
      ),
    );
    ...
    expect((await res.json()).linkset[0]["service-desc"][0].href).toContain(
      "/openapi.json",
    );
  });

  it("publishes an MCP Server Card and Agent Skills index", async () => {
    const card = requireResponse(
      await onRequest(
        ctxFor("/.well-known/mcp/server-card.json") as never,
        next as never,
      ),
    );
    expect((await card.json()).transport.endpoint).toBe(
      "https://rubenmarcus.dev/api/mcp",
    );

    const index = requireResponse(
      await onRequest(
        ctxFor("/.well-known/agent-skills/index.json") as never,
        next as never,
      ),
    );
    expect((await index.json()).skills[0].digest).toMatch(
      /^sha256:[a-f0-9]{64}$/,
    );
  });
  ...
  it("adds discovery Link headers on the homepage", async () => {
    const res = requireResponse(
      await onRequest(ctxFor("/") as never, next as never),
    );
  ...
  it("negotiates rendered HTML to Markdown", async () => {
    ...
    const res = requireResponse(
      await onRequest(
        ...
        (() =>
          new Response(
            "<html><head><title>About</title></head><main><h1>Hello</h1><p>World</p></main></html>",
            {
...
describe("curl easter egg", () => {
  it("answers terminal clients on page routes with the text resume directly", async () => {
    ...
    const res = requireResponse(
      await onRequest(ctxFor("/", "curl/8.0") as never, next as never),
    );
  ...
  it("leaves API routes alone for terminal clients", async () => {
    const res = requireResponse(
      await onRequest(ctxFor("/api/mcp", "curl/8.0") as never, next as never),
    );
  ...
  it("leaves browsers alone on page routes", async () => {
    const res = requireResponse(
      await onRequest(ctxFor("/about") as never, next as never),
    );
    expect(await res.text()).toBe("next");
  });
});
