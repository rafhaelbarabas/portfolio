...
describe("first-touch attribution", () => {
  it("groups LinkedIn host variants into one acquisition source", () => {
    ...
    expect(normalizeSource("android-app://com.linkedin.android")).toBe(
      "linkedin",
    );
  });
  ...
  it("groups other strategic referrers", () => {
    expect(
      normalizeSource("https://www.awwwards.com/sites/rubenmarcus-dev"),
    ).toBe("awwwards");
    expect(normalizeSource("https://t.co/example")).toBe("x");
    expect(normalizeSource("https://claude.ai/new")).toBe("claude");
    expect(normalizeSource("https://www.reddit.com/r/SideProject")).toBe(
      "reddit",
    );
    expect(normalizeSource("https://br.pinterest.com/pin/123")).toBe(
      "pinterest",
    );
    expect(normalizeSource("https://land-book.com/websites/123")).toBe(
      "landbook",
    );
    expect(normalizeSource("https://www.behance.net/gallery/123")).toBe(
      "behance",
    );
    expect(normalizeSource("https://www.indiehackers.com/post/example")).toBe(
      "indie_hackers",
    );
  });
  ...
  it("prefers an explicit UTM source and preserves the landing campaign", () => {
    const attribution = buildFirstTouchAttribution(
      new URL(
        "https://rubenmarcus.dev/services/aeo?utm_source=siteinspire&utm_medium=directory&utm_campaign=portfolio-launch&utm_content=agent-ready",
      ),
