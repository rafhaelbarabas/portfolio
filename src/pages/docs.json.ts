  ...
  const documents = [
    {
      ...
      title: "Ruben Marcus",
      description:
        "AI Fullstack Engineer building AI products, agent systems, AEO infrastructure, and high-performance web experiences.",
      language: "en",
    },
    {
      url: absolute("/pt/"),
      title: "Ruben Marcus",
      description:
        "Engenheiro AI Fullstack construindo produtos de IA, sistemas de agents, infraestrutura AEO e experiências web de alta performance.",
      language: "pt-BR",
    },
    ...serviceOffers.flatMap((service) => [
      ...
      {
        url: absolute(`/pt/services/${service.slug}`),
        title: service.name.pt,
    ...
    ...outcomeCaseStudies.flatMap((study) => [
      {
        url: absolute(`/work/${study.slug}`),
        title: study.title.en,
        description: study.description.en,
        language: "en",
      },
      {
        url: absolute(`/pt/work/${study.slug}`),
        title: study.title.pt,
        description: study.description.pt,
        language: "pt-BR",
      },
    ]),
    ...agentSkills.map((skill) => ({
      url: absolute(`/skills#${skill.slug}`),
      title: skill.name,
      description: skill.summary.en,
      language: "en",
    })),
    ...english.map((post) => ({
      url: absolute(getBlogPaths(post.id).en),
      title: post.data.title,
      description: post.data.description,
      language: "en",
      lastModified: (post.data.updated ?? post.data.date).toISOString(),
    })),
    ...portuguese.map((post) => ({
      url: absolute(getBlogPaths(post.id).pt),
      title: post.data.title,
      description: post.data.description,
      language: "pt-BR",
      lastModified: (post.data.updated ?? post.data.date).toISOString(),
