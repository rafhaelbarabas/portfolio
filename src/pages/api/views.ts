...
export const GET: APIRoute = async ({ url }) => {
  ...
  if (slug === null) {
    ...
    const rows = await sbSelect<{ slug: string; views: number }>(
      "page_views",
      "select=slug,views",
    );
    const counters = Object.fromEntries(
      rows.map((row) => [row.slug, row.views]),
    );
  ...
  const rows = await sbSelect<{ views: number }>(
    "page_views",
    `slug=eq.${encodeURIComponent(slug)}&select=views`,
  );
