...
const run = async (): Promise<void> => {
  const nodes = Array.from(
    document.querySelectorAll<HTMLElement>("[data-post-stats]"),
  );
  ...
  try {
    ...
    for (const node of nodes) {
      ...
      if (typeof views[slug] === "number")
        parts.push(`${views[slug].toLocaleString("en-US")} views`);
      if (typeof likes[slug] === "number" && likes[slug] > 0)
        parts.push(`♥ ${likes[slug].toLocaleString("en-US")}`);
      if (parts.length > 0) {
